...
object Player {
  val players = Map(
    ...
    PlayersEnum.PulsePlayer         -> PulsePlayer,
    PlayersEnum.ComprehensivePlayer -> ComprehensivePlayer
...
object PulsePlayer extends Player[NoteWithAttributes] {
  ...
  override def playNote(c: Context[NoteWithAttributes], d: Duration, n: NoteWithAttributes): Performance = {
    val pulsifiedVolume: Types.Volume = {
      val accent =
        c.cTimeSignature.pulse.calculateAccent(c.cTime, c.cTimeSignature.getTickedBeat(), c.cTimeSignature.startTime)
      accent match {
